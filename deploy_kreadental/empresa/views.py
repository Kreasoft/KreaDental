...
def listar_usuarios_empresa(request, empresa_id):
    ...
    # Para superusuarios, acceso directo sin verificaciones
...
def crear_usuario_empresa(request, empresa_id):
    ...
    if request.user.is_superuser:
        ...
        if not usuario_actual:
            messages.error(request, 'No tienes permisos para crear usuarios en esta empresa.')
        ...
        if usuario_actual.tipo_usuario not in ['super_admin', 'admin_empresa']:
            messages.error(request, f'No tienes permisos para crear usuarios. Tu rol actual es: {usuario_actual.tipo_usuario}')
            return redirect('empresa:listar_usuarios_empresa', empresa_id=empresa.id)
    ...
    # Limpiar mensajes de error acumulados
...
def editar_usuario_empresa(request, empresa_id, usuario_empresa_id):
    ...
    # Un usuario no puede editarse a sí mismo (excepto superusuarios)
    if usuario_empresa.usuario == request.user and not request.user.is_superuser:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                'success': False,

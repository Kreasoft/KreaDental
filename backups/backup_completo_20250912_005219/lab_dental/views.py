...
def detalle_trabajo(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Si hay motivo de devolución, agregarlo a las notas
            motivo_devolucion = form.cleaned_data.get('motivo_devolucion')
            if motivo_devolucion and seguimiento.estado == 'DEVUELTO':
                motivo_texto = dict(form.MOTIVO_DEVOLUCION_CHOICES)[motivo_devolucion]
                seguimiento.notas = f"MOTIVO DE DEVOLUCIÓN: {motivo_texto}\n\n{seguimiento.notas}"
            ...
            if seguimiento.estado == 'DEVUELTO':
                messages.warning(request, f'Trabajo devuelto al laboratorio. Motivo: {motivo_texto if motivo_devolucion else "No especificado"}')
            elif seguimiento.estado == 'ENTREGADO':
                messages.success(request, '¡Trabajo entregado al paciente! Proceso completado.')
            else:

...
from .models import PagoTratamiento, HistorialPago, PagoAtencion
from .nuevas_vistas import editar_pago_atencion, anular_pago_atencion
from tratamientos.models import Tratamiento
from formas_pago.models import FormaPago
from citas.models import Cita
from django.contrib.humanize.templatetags.humanize import intcomma

def procesar_monto(monto_str):
    """
    Procesa un monto desde el formulario, manejando formato chileno
    """
    if not monto_str:
        return None
    
    # Limpiar el valor del monto
    ...
    # Si tiene punto como separador de miles, removerlo
    elif '.' in monto_str and ',' not in monto_str:
        # Verificar si es separador de miles (más de 2 dígitos después del punto)
...
def crear_pago(request, tratamiento_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Verificar si el tratamiento está completamente pagado
            total_pagado = PagoTratamiento.objects.filter(
                tratamiento=tratamiento,
                estado='COMPLETADO'
            ).aggregate(total=models.Sum('monto'))['total'] or 0
            
            if total_pagado >= tratamiento.costo_total and tratamiento.estado != 'COMPLETADO':
...
@login_required
def agregar_pago_simple(request):
    """Vista simplificada para agregar pagos"""
    if request.method == 'POST':
        try:
            ...
            if not tratamiento_id or not monto_str or not forma_pago_id:
                return JsonResponse({
                    'status': 'error',
            ...
            if monto > saldo_pendiente:
                return JsonResponse({
                    'status': 'error',
                    'message': f'El monto no puede exceder el saldo pendiente (${saldo_pendiente:,.0f})'
                })
    ...
    # Obtener tratamientos con saldo pendiente
    tratamientos = Tratamiento.objects.annotate(
        ...
        estado='COMPLETADO'
    ).order_by('-fecha_creacion')[:20]  # Limitar a 20 tratamientos más recientes
    ...
    return render(request, 'pagos_tratamientos/agregar_pago_simple.html', context)

@login_required
def lista_pagos_atenciones(request):
    """Lista de pagos de atenciones únicas"""
    pagos = PagoAtencion.objects.all().order_by('-fecha_pago')
    ...
    if cita_id:
        ...
        citas_disponibles = None
    else:
        cita = None
        # Obtener citas que no tengan pagos completados
        citas_disponibles = Cita.objects.filter(
            estado='COMPLETADA'
        ).exclude(
            pagos_atencion__estado='COMPLETADO'
        ).order_by('-fecha')[:20]
    ...
    # Obtener atenciones pendientes de pago (citas completadas sin pagos)
    atenciones_pendientes = Cita.objects.filter(
        estado='COMPLETADA'
    ).exclude(
        pagos_atencion__estado='COMPLETADO'
    ).order_by('-fecha')[:10]
    ...
    return render(request, 'pagos_tratamientos/lista_pagos_atenciones.html', context)

@login_required
def agregar_pago_atencion(request, cita_id):
    """Agregar pago para una atención única (cita)"""
    cita = get_object_or_404(Cita, id=cita_id)
    formas_pago = FormaPago.objects.filter(estado=True)
    ...
    if request.method == 'POST':
        try:
            monto_str = request.POST.get('monto', '').strip()
            forma_pago_id = request.POST.get('forma_pago')
            tipo_pago = request.POST.get('tipo_pago', 'ATENCION_UNICA')
            comprobante = request.POST.get('comprobante', '')
            notas = request.POST.get('notas', '')
            ...
            # Crear el pago
            pago = PagoAtencion.objects.create(
                cita=cita,
                forma_pago=forma_pago,
                monto=monto,
                tipo_pago=tipo_pago,
            ...
            return JsonResponse({
                'status': 'success',
                'message': 'Pago de atención registrado exitosamente',
                'pago_id': pago.id
            })
    ...
    return render(request, 'pagos_tratamientos/agregar_pago_atencion.html', context)

@login_required
def agregar_pago_atencion_rapido(request):
    """Vista rápida para agregar pagos de atenciones"""
    if request.method == 'POST':
        try:
            ...
            # Validar datos requeridos
            if not cita_id or not monto_str or not forma_pago_id:
                return JsonResponse({
                    'status': 'error',
                    'message': 'Todos los campos son obligatorios'
                })
            ...
            return JsonResponse({
                'status': 'success',
                'message': f'Pago de ${monto:,.0f} registrado para {cita.paciente.nombre_completo}',
                'pago_id': pago.id
            })
    ...
    # GET request - obtener atenciones pendientes
    atenciones_pendientes = Cita.objects.filter(
        estado='COMPLETADA'
    ).exclude(
        pagos_atencion__estado='COMPLETADO'
    ...
    return render(request, 'pagos_tratamientos/agregar_pago_atencion_rapido.html', context)

@login_required
def estado_cuenta_paciente(request, paciente_id):
    """Estado de cuenta completo del paciente"""
    ...
    try:
        ...
        citas = Cita.objects.filter(paciente=paciente, estado='COMPLETADA')
        
        # Calcular totales
        total_tratamientos = tratamientos.aggregate(total=Sum('costo_total'))['total'] or 0
        total_pagado_tratamientos = PagoTratamiento.objects.filter(
            tratamiento__paciente=paciente,
            estado='COMPLETADO'
        ).aggregate(total=Sum('monto'))['total'] or 0
        
        total_pagado_atenciones = PagoAtencion.objects.filter(
            cita__paciente=paciente,
            estado='COMPLETADO'
        ).aggregate(total=Sum('monto'))['total'] or 0
        
        saldo_pendiente = total_tratamientos - total_pagado_tratamientos
        total_pagado_general = total_pagado_tratamientos + total_pagado_atenciones
